...
struct WhiteBalanceControl: View {
    ...
    func valueChanged() {
        ...
        let max = FilterWhiteBalance.range.max
        let ratio = max/Double(100)
        
        let valueTemperature = self.temperatureIntensity * ratio
        
        let valueTint = self.tintIntensity * ratio
